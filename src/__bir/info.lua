...
local function _get_bir_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.birakecoind == 'running' then
    ...
    local _exitcode, _stdout, _stderr = _exec_bir_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_bir_cli_result(_exitcode, _stdout, _stderr)
